...
public class BankAccountImpl implements BankAccount {
	...
	public void tickMetronome() {
		...
		if (activeBookCounts >= 1) {
			...
			this.cumulativeEntropy += 1; // Ordinal leap entropy
			balance = io.github.uwol.compecon.economy.arithmodynamics.TopologyEngine.getPrimeValueForCounts(primeCounts); // Re-sync balance
		}
		...
		while (balance >= io.github.uwol.compecon.economy.arithmodynamics.TopologyEngine.MINT_SCARCITY) {
			...
			this.cumulativeEntropy += (primeCounts - oldCounts); // Phase transition entropy (negative)
		}
	}
